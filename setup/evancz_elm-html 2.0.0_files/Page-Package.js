...
Elm.ColorScheme.make = function (_elm) {
   ...
   if (_elm.ColorScheme.values)
   return _elm.ColorScheme.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   132,
   206);
   var greyBlue = A3($Color.rgb,
   90,
   99,
   120);
   _elm.ColorScheme.values = {_op: _op
                             ,greyBlue: greyBlue
                             ,blue: blue
                             ,yellow: yellow
                             ,pink: pink
...
Elm.Component.Documentation = Elm.Component.Documentation || {};
Elm.Component.Documentation.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.Documentation = _elm.Component.Documentation || {};
   if (_elm.Component.Documentation.values)
   return _elm.Component.Documentation.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Component.Documentation",
   $Basics = Elm.Basics.make(_elm),
   ...
   var sandwich = F3(function (start,
   stop,
   txt) {
      return A2($Basics._op["++"],
      $Text.fromString(start),
      A2($Basics._op["++"],
      txt,
      $Text.fromString(stop)));
   });
   ...
   var accentString = function (str) {
      return A2($Text.color,
      $ColorScheme.green,
      $Text.fromString(str));
   };
   var colon = pad(accentString(":"));
   var arrow = pad(accentString("->"));
   var bar = pad(accentString("|"));
   var equals = pad(accentString("="));
   var space = $Text.fromString(" ");
   var spacePrefix = function (txts) {
      return $Text.concat(A2($List.map,
      function (txt) {
         return A2($Basics._op["++"],
         space,
         txt);
   ...
   var isTuple = function (str) {
      return function () {
         var _v0 = $String.toInt(A2($String.dropLeft,
         6,
         str));
         switch (_v0.ctor)
         {case "Err": return false;
            case "Ok":
            return _U.eq(A2($String.left,
              6,
              str),
              "_Tuple");}
         _U.badCase($moduleName,
   ...
   var ADT = {ctor: "ADT"};
   var None = {ctor: "None"};
   var viewType = function (tipe) {
      return A2(viewTypeHelp,
      None,
      tipe);
   };
   var viewTypeHelp = F2(function (context,
   tipe) {
      return function () {
         switch (tipe.ctor)
         {case "App":
            switch (tipe._0.ctor)
              {case "Type":
                 return _U.eq(tipe._0._0,
                   "_List") && _U.eq($List.length(tipe._1),
                   1) ? A3(sandwich,
                   "[",
                   "]",
                   viewType($List.head(tipe._1))) : isTuple(tipe._0._0) ? parens($Text.concat($List.intersperse($Text.fromString(", "))(A2($List.map,
                   viewType,
                   tipe._1)))) : function () {
                      var txt = A2($Basics._op["++"],
                      $Text.fromString(tipe._0._0),
                      spacePrefix(A2($List.map,
                      viewTypeHelp(ADT),
                      tipe._1)));
                      return function () {
            ...
            return function () {
                 ...
                 return function () {
                    switch (context.ctor)
                    {case "None": return txt;}
                    return parens(txt);
                 }();
              }();
            case "Record":
            return function () {
                 var viewExtension = function (maybeType) {
                    return function () {
                       switch (maybeType.ctor)
                       {case "Just":
                          return A2($Basics._op["++"],
                            viewType(maybeType._0),
                            $Text.fromString(" | "));
                          case "Nothing":
                 ...
                 var viewField = function (_v21) {
                    return function () {
                       switch (_v21.ctor)
                       {case "_Tuple2":
                          return A2($Basics._op["++"],
                            $Text.fromString(_v21._0),
                            A2($Basics._op["++"],
                            colon,
                            viewType(_v21._1)));}
                       _U.badCase($moduleName,
                       "on line 324, column 17 to 63");
                    }();
                 };
                 return A2(sandwich,
                 "{ ",
                 " }")($Text.concat(_L.fromArray([viewExtension(tipe._1)
                                                 ,$Text.join($Text.fromString(", "))(A2($List.map,
                                                 viewField,
                                                 tipe._0))])));
              }();
            case "Type":
            return $Text.fromString(tipe._0);
            case "Var":
            return $Text.fromString(tipe._0);}
         _U.badCase($moduleName,
         "between lines 291 and 336");
      }();
   });
   var isVarChar = function (c) {
      return $Char.isLower(c) || ($Char.isUpper(c) || ($Char.isDigit(c) || (_U.eq(c,
      _U.chr("_")) || _U.eq(c,
      _U.chr("\'")))));
   };
   var viewVar = function (str) {
      return function () {
         var txt = $Text.fromString(str);
         return function () {
            var _v25 = $String.uncons(str);
            switch (_v25.ctor)
            {case "Just":
               switch (_v25._0.ctor)
                 {case "_Tuple2":
                    return isVarChar(_v25._0._0) ? txt : parens(txt);}
                 break;
   ...
   var viewValue = function (value) {
      return $Text.concat(_L.fromArray([A2($Text.link,
                                       A2($Basics._op["++"],
                                       "#",
                                       value.name),
      ...
      return function () {
         ...
         {case "_Tuple2":
            return A2($Basics._op["++"],
              ...
              A2($Basics._op["++"],
              accentString(sep),
              spacePrefix(A2($List._op["::"],
              $Text.fromString(_v29._0),
              A2($List.map,
              viewTypeHelp(ADT),
              _v29._1)))));}
         _U.badCase($moduleName,
   ...
   var viewUnion = function (union) {
      return function () {
         var seperators = A2($List._op["::"],
         "=",
         A2($List.repeat,
         $List.length(union.cases) - 1,
         "|"));
         return $Text.concat(_L.fromArray([accentString("type ")
                                          ,A2($Text.link,
                                          A2($Basics._op["++"],
                                          "#",
                                          union.name),
                                          ...
                                          $Text.fromString,
                                          union.args))
                                          ,$Text.concat(A3($List.map2,
                                          viewCase,
                                          seperators,
                                          union.cases))]));
      }();
   };
   var viewAlias = function (alias) {
      return $Text.concat(_L.fromArray([accentString("type alias ")
                                       ,A2($Text.link,
                                       A2($Basics._op["++"],
                                       "#",
                                       alias.name),
   ...
   var options = function () {
      var defaults = $Markdown.defaultOptions;
      return _U.replace([["sanitize"
                         ,true]],
      defaults);
   }();
   var viewEntry = F3(function (innerWidth,
   name,
   _v33) {
      return function () {
         switch (_v33.ctor)
         {case "_Tuple3":
            return function () {
                 var commentElement = $String.isEmpty(_v33._2) ? $Graphics$Element.empty : A2($Graphics$Element.flow,
                 ...
                 _L.fromArray([A2($Graphics$Element.spacer,
                              ...
                              innerWidth - 40,
                              A2($Markdown.toElementWith,
                              options,
                              _v33._2))]));
                 var annotationPadding = 10;
                 var rawAssocPrec = function () {
                    switch (_v33._1.ctor)
                    {case "Just":
                       switch (_v33._1._0.ctor)
                         {case "_Tuple2":
                            return $Text.rightAligned($Text.height(12)($Text.fromString(A2($Basics._op["++"],
                              _v33._1._0._0,
                              A2($Basics._op["++"],
                              "-associative / precedence ",
                              $Basics.toString(_v33._1._0._1))))));}
                         break;
                       case "Nothing":
                       return $Graphics$Element.empty;}
                    _U.badCase($moduleName,
                    "between lines 176 and 184");
                 }();
                 var assocPrecWidth = $Graphics$Element.widthOf(rawAssocPrec) + 20;
                 var annotationWidth = innerWidth - annotationPadding - assocPrecWidth;
                 var annotationText = $Graphics$Element.width(annotationWidth)($Text.leftAligned($Text.monospace(_v33._0)));
                 var annotationHeight = $Graphics$Element.heightOf(annotationText) + 8;
                 var assocPrec = A4($Graphics$Element.container,
                 assocPrecWidth,
                 A2($Basics.min,
                 annotationHeight,
                 24),
                 ...
                 rawAssocPrec);
                 var annotationBar = A2($Graphics$Element.flow,
                 $Graphics$Element.right,
                 _L.fromArray([A2($Graphics$Element.spacer,
                              annotationPadding,
                              annotationHeight)
                 ...
                 _L.fromArray([A2($Graphics$Element.tag,
                              name,
                              A2($Graphics$Element.color,
                              $ColorScheme.mediumGrey,
      ...
      return function () {
         ...
         {case "app":
            ...
            return A2($Json$Decode.object1,
              ...
              $Json$Decode.string));}
         _U.badCase($moduleName,
         "between lines 138 and 158");
      }();
   };
   var tipe = A2($Json$Decode.andThen,
   A2($Json$Decode._op[":="],
   "tag",
   $Json$Decode.string),
   specificType);
   var assocPrec = $Json$Decode.maybe(A3($Json$Decode.object2,
   F2(function (v0,v1) {
      return {ctor: "_Tuple2"
             ,_0: v0
             ,_1: v1};
   ...
   var value = A5($Json$Decode.object4,
   Value,
   A2($Json$Decode._op[":="],
   "name",
   $Json$Decode.string),
   ...
   var union = A5($Json$Decode.object4,
   Union,
   A2($Json$Decode._op[":="],
   "name",
   $Json$Decode.string),
   ...
   var alias = A5($Json$Decode.object4,
   Alias,
   A2($Json$Decode._op[":="],
   "name",
   $Json$Decode.string),
   ...
   var valueList = function () {
      var nameList = $Json$Decode.list(A2($Json$Decode._op[":="],
      "name",
      $Json$Decode.string));
      var allNames = A4($Json$Decode.object3,
      F3(function (x,y,z) {
         return A2($Basics._op["++"],
         x,
         A2($Basics._op["++"],y,z));
   ...
   var Documentation = F5(function (a,
   b,
   c,
   d,
   e) {
   ...
   var documentation = A6($Json$Decode.object5,
   Documentation,
   A2($Json$Decode._op[":="],
   "name",
   $Json$Decode.string),
   ...
   A2($Json$Decode._op[":="],
   "aliases",
   $Json$Decode.list(alias)),
   A2($Json$Decode._op[":="],
   "types",
   $Json$Decode.list(union)),
   A2($Json$Decode._op[":="],
   "values",
   $Json$Decode.list(value)));
   var toDocDict = function (docs) {
      return function () {
         var toPairs = F3(function (view,
         getAssocPrec,
         entries) {
            return A2($List.map,
            function (entry) {
   ...
   _elm.Component.Documentation.values = {_op: _op
                                         ,toDocDict: toDocDict
...
Elm.Component = Elm.Component || {};
Elm.Component.DropDown = Elm.Component.DropDown || {};
Elm.Component.DropDown.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.DropDown = _elm.Component.DropDown || {};
   if (_elm.Component.DropDown.values)
   return _elm.Component.DropDown.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Component.DropDown",
   $Basics = Elm.Basics.make(_elm),
   ...
   var nth = F2(function (_v0,n) {
      return function () {
         ...
         {case "::": return _U.eq(n,
              ...
              n - 1);}
         _U.badCase($moduleName,
         "on line 34, column 3 to 39");
      }();
   });
   var viewOption = F2(function (current,
   string) {
      return A2($Html.option,
      _L.fromArray([$Html$Attributes.value(string)
                   ,$Html$Attributes.selected(_U.eq(current,
                   string))]),
      _L.fromArray([$Html.text(string)]));
   });
   var dropdown = F3(function (updates,
   current,
   options) {
      return A2($Html.div,
      ...
      _L.fromArray([$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                   ...
                   $Json$Decode.$int),
                   function ($) {
                      return $LocalChannel.send(updates)(nth(options)($));
                   })]),
      A2($List.map,
      viewOption(current),
      options))]));
   });
   _elm.Component.DropDown.values = {_op: _op
                                    ,dropdown: dropdown
...
Elm.Component.Header.make = function (_elm) {
   ...
   if (_elm.Component.Header.values)
   return _elm.Component.Header.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Component.Header",
   $Basics = Elm.Basics.make(_elm),
   $Component$DropDown = Elm.Component.DropDown.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Html = Elm.Html.make(_elm),
   $LocalChannel = Elm.LocalChannel.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Text = Elm.Text.make(_elm);
   var view = F7(function (versionChan,
   innerWidth,
   user,
   $package,
   version,
   ...
   maybeModule) {
      return function () {
         ...
         "/tree/",
         version)))));
         var viewSource = $Text.centered($Text.height(12)($Text.link(githubLink)($Text.fromString("View Source"))));
         var packageLink = function () {
         ...
         var userLink = $Text.fromString(user);
         var userPackageText = A2($Basics._op["++"],
         userLink,
         A2($Basics._op["++"],
         $Text.fromString(" / "),
         packageLink));
         var headerText = function () {
            ...
            {case "Just":
               return A2($Basics._op["++"],
                 ...
                 maybeModule._0)));
               case "Nothing":
               return userPackageText;}
            _U.badCase($moduleName,
            "between lines 31 and 35");
         }();
         var bigWords = $Text.leftAligned($Text.height(24)(headerText));
         return A2($Graphics$Element.flow,
         $Graphics$Element.right,
         _L.fromArray([A4($Graphics$Element.container,
                      ...
                      $Graphics$Element.down,
                      _L.fromArray([A3($Html.toElement,
                                   100,
                                   30,
                                   A3($Component$DropDown.dropdown,
                                   versionChan,
                                   version,
                                   versions))
                                   ,A2($Graphics$Element.width,
   ...
   _elm.Component.Header.values = {_op: _op
                                  ,view: view};
   return _elm.Component.Header.values;
};
Elm.Component = Elm.Component || {};
Elm.Component.Package = Elm.Component.Package || {};
Elm.Component.Package.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.Package = _elm.Component.Package || {};
   if (_elm.Component.Package.values)
   return _elm.Component.Package.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Component.Package",
   $Basics = Elm.Basics.make(_elm),
   $ColorScheme = Elm.ColorScheme.make(_elm),
   $Component$Header = Elm.Component.Header.make(_elm),
   $Component$Package$ModuleList = Elm.Component.Package.ModuleList.make(_elm),
   $Component$Package$Search = Elm.Component.Package.Search.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $LocalChannel = Elm.LocalChannel.make(_elm),
   $Markdown = Elm.Markdown.make(_elm),
   $Maybe = Elm.Maybe.make(_elm);
   ...
   var view = F6(function (versionChan,
   searchChan,
   w,
   pkg,
   searchTerm,
   maybeReadme) {
      return A2($Graphics$Element.flow,
      $Graphics$Element.down,
      _L.fromArray([A7($Component$Header.view,
                   versionChan,
                   w,
                   pkg.user,
                   pkg.name,
                   ...
                   _L.fromArray([A2($Graphics$Element.spacer,
                                30,
                                5)
                                ,A3($Component$Package$ModuleList.view,
                                w - 30 - 200 - 30,
                                searchTerm,
                                pkg)
                                ,A4($Component$Package$Search.view,
                                searchChan,
                                200,
                                $Component$Package$ModuleList.isCore(pkg),
                                searchTerm)]))
                   ,A2($Graphics$Element.spacer,
                   w,
                   12)
   ...
   _elm.Component.Package.values = {_op: _op
                                   ,view: view
                                   ,options: options};
   return _elm.Component.Package.values;
};
Elm.Component = Elm.Component || {};
Elm.Component.Package = Elm.Component.Package || {};
Elm.Component.Package.ModuleList = Elm.Component.Package.ModuleList || {};
Elm.Component.Package.ModuleList.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.Package = _elm.Component.Package || {};
   _elm.Component.Package.ModuleList = _elm.Component.Package.ModuleList || {};
   if (_elm.Component.Package.ModuleList.values)
   return _elm.Component.Package.ModuleList.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Component.Package.ModuleList",
   $Basics = Elm.Basics.make(_elm),
   $ColorScheme = Elm.ColorScheme.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $List = Elm.List.make(_elm),
   ...
   $String = Elm.String.make(_elm),
   $Text = Elm.Text.make(_elm);
   var searchModule = F2(function (searchTerm,
   _v0) {
      return function () {
         switch (_v0.ctor)
         {case "_Tuple2":
            return function () {
                 var _v4 = A2($List.filter,
                 function ($) {
                    return $String.contains(searchTerm)($String.toLower($));
                 },
                 _v0._1);
                 switch (_v4.ctor)
                 {case "[]":
                    return A2($String.contains,
                      searchTerm,
                      $String.toLower(_v0._0)) ? $Maybe.Just({ctor: "_Tuple2"
                                                             ,_0: _v0._0
                                                             ,_1: _L.fromArray([])}) : $Maybe.Nothing;}
   ...
   var search = F2(function (searchTerm,
   modules) {
      return A2($List.filterMap,
      searchModule(searchTerm),
      modules);
   });
   var notFound = function (term) {
      return $Text.leftAligned($Text.color($ColorScheme.mediumGrey)(A2($Basics._op["++"],
      $Text.fromString("No matches for "),
      $Text.monospace($Text.fromString(term)))));
   };
   var link = F4(function (width,
   url,
   padding,
   txt) {
      ...
      width,
      24,
      $Graphics$Element.midLeft)($Text.leftAligned(A2($Basics._op["++"],
      $Text.monospace($Text.fromString(padding)),
      A2($Text.link,url,txt))));
   });
   var viewModule = F4(function (width,
   rootUrl,
   searchStatus,
   _v5) {
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 var viewValue = function (name) {
                    ...
                    A2($Basics._op["++"],"#",name)),
                    "  ",
                    $Text.monospace($Text.fromString(name)));
                 };
                 var name = A4(link,
                 width,
                 url,
                 "",
                 $Text.fromString(_v5._0));
                 return function () {
                    switch (searchStatus.ctor)
                    {case "No": return name;
                       case "Yes":
                       return A2($Graphics$Element.flow,
                         $Graphics$Element.down,
   ...
   var No = {ctor: "No"};
   var Yes = {ctor: "Yes"};
   var isCore = function (model) {
      return _U.eq(model.user,
      "elm-lang") && _U.eq(model.name,
      "core");
   };
   var toUrl = function (model) {
      return A2($Basics._op["++"],
      "/packages/",
      A2($Basics._op["++"],
      model.user,
   ...
   model) {
      return function () {
         var $ = function () {
            var _v10 = $String.trim(searchTerm);
            switch (_v10)
            {case "":
               return {ctor: "_Tuple2"
                      ,_0: No
   ...
   _elm.Component.Package.ModuleList.values = {_op: _op
                                              ,view: view
                                              ,isCore: isCore
                                              ,Model: Model};
   return _elm.Component.Package.ModuleList.values;
...
Elm.Component = Elm.Component || {};
Elm.Component.Package = Elm.Component.Package || {};
Elm.Component.Package.Search = Elm.Component.Package.Search || {};
Elm.Component.Package.Search.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.Package = _elm.Component.Package || {};
   _elm.Component.Package.Search = _elm.Component.Package.Search || {};
   if (_elm.Component.Package.Search.values)
   return _elm.Component.Package.Search.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Component.Package.Search",
   $Basics = Elm.Basics.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   $Html = Elm.Html.make(_elm),
   $Html$Attributes = Elm.Html.Attributes.make(_elm),
   ...
   var inputStyle = $Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                        ...
                                                        ,{ctor: "_Tuple2"
                                                         ,_0: "border-radius"
                                                         ,_1: "4px"}]));
   var description = function (isCore) {
      return function () {
         var hint = isCore ? " Try searching for `|>` or `map`." : "";
   ...
   var view = F4(function (fieldChan,
   width,
   isCore,
   fieldContent) {
      ...
      _L.fromArray([A2($Html.input,
                   _L.fromArray([$Html$Attributes.placeholder("Package Search")
                                ,$Html$Attributes.value(fieldContent)
                                ,A3($Html$Events.on,
                                "input",
                                $Html$Events.targetValue,
                                $LocalChannel.send(fieldChan))
                                ,inputStyle]),
                   _L.fromArray([]))
                   ,A2($Html$Lazy.lazy,
                   description,
   ...
   _elm.Component.Package.Search.values = {_op: _op
                                          ,view: view};
   return _elm.Component.Package.Search.values;
};
Elm.Component = Elm.Component || {};
Elm.Component.TopBar = Elm.Component.TopBar || {};
Elm.Component.TopBar.make = function (_elm) {
   "use strict";
   _elm.Component = _elm.Component || {};
   _elm.Component.TopBar = _elm.Component.TopBar || {};
   if (_elm.Component.TopBar.values)
   return _elm.Component.TopBar.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "Component.TopBar",
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $ColorScheme = Elm.ColorScheme.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   var targetWidth = 980;
   var topBarHeight = 50;
   var bar = F2(function (fillerWidth,
   elem) {
   ...
   var view = function (outerWidth) {
      return function () {
         var innerWidth = A2($Basics.min,
         outerWidth,
         targetWidth);
         var leftPadding = (outerWidth - innerWidth) / 2 | 0;
         var rightPadding = outerWidth - innerWidth - leftPadding;
         return A2($Graphics$Element.flow,
         $Graphics$Element.right,
         _L.fromArray([A2(bar,
                      leftPadding,
                      ...
                      A3($Color.rgb,5,80,129),
                      $Text.fromString("Packages")))))
                      ,A2(bar,
                      innerWidth - logoSize - searchBarWidth + rightPadding,
                      $Graphics$Element.empty)]));
      }();
   };
   _elm.Component.TopBar.values = {_op: _op
...
Elm.Page.Package.make = function (_elm) {
   "use strict";
   _elm.Page = _elm.Page || {};
   _elm.Page.Package = _elm.Page.Package || {};
   if (_elm.Page.Package.values)
   return _elm.Page.Package.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   $Basics = Elm.Basics.make(_elm),
   $ColorScheme = Elm.ColorScheme.make(_elm),
   $Component$Documentation = Elm.Component.Documentation.make(_elm),
   $Component$Package = Elm.Component.Package.make(_elm),
   $Component$Package$ModuleList = Elm.Component.Package.ModuleList.make(_elm),
   $Component$TopBar = Elm.Component.TopBar.make(_elm),
   $Graphics$Element = Elm.Graphics.Element.make(_elm),
   ...
   var versionChan = $Signal.channel("");
   var searchChan = $Signal.channel("");
      ...
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 980,
                 _v0._0);
                 return $Graphics$Element.color($ColorScheme.background)(A2($Graphics$Element.flow,
                 $Graphics$Element.down,
                 _L.fromArray([$Component$TopBar.view(_v0._0)
                              ,A2($Graphics$Element.flow,
                              $Graphics$Element.right,
                              _L.fromArray([A2($Graphics$Element.spacer,
                                           (_v0._0 - innerWidth) / 2 | 0,
                                           _v0._1 - $Component$TopBar.topBarHeight)
                                           ,A6($Component$Package.view,
         ...
         _U.badCase($moduleName,
         "between lines 103 and 118");
      }();
   });
   var extractReadme = function (response) {
      return function () {
         switch (response.ctor)
         {case "Success":
            return $Maybe.Just(response._0);}
         return $Maybe.Nothing;
      }();
   };
   ...
   function (v) {
      return typeof v === "object" && "user" in v && "name" in v && "version" in v && "versionList" in v ? {_: {}
                                                                                                           ,user: typeof v.user === "string" || typeof v.user === "object" && v.user instanceof String ? v.user : _U.badPort("a string",
                                                                                                           v.user)
                                                                                                           ...
                                                                                                           v.version)
                                                                                                           ,versionList: typeof v.versionList === "object" && v.versionList instanceof Array ? Elm.Native.List.make(_elm).fromArray(v.versionList.map(function (v) {
                                                                                                              return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",
                                                                                                              v);
                                                                                                           })) : _U.badPort("an array",
                                                                                                           v.versionList)} : _U.badPort("an object with fields \'user\', \'name\', \'version\', \'versionList\'",
      v);
   });
   ...
   " ",
   context.version)))));
   var packageUrl = function (version) {
      return A2($Basics._op["++"],
      "/packages/",
      A2($Basics._op["++"],
      context.user,
   ...
   var redirect = _P.portOut("redirect",
   _P.outgoingSignal(function (v) {
      return v;
   }),
   $Signal.map(packageUrl)(A3($Signal.keepIf,
   F2(function (x,y) {
      return !_U.eq(x,y);
   })(""),
   "",
      ...
      return function () {
         ...
         {case "Success":
            return function () {
                 ...
                 $Json$Decode.list($Component$Documentation.valueList),
                 response._0);
                 switch (_v8.ctor)
                 {case "Err":
                    return packageInfo(_L.fromArray([]));
                    case "Ok":
                    return packageInfo(_v8._0);}
                 _U.badCase($moduleName,
   ...
   $Window.dimensions,
   moduleList,
   $Signal.subscribe(searchChan),
   readme);
   _elm.Page.Package.values = {_op: _op
